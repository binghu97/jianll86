...
sleep 8s
rm /opt/tomcatidc/ipscpdiskcleanmulu/* -f
sed  -e 's/[ ]*$//g' /opt/tomcatidc/ipscpdiskcleanpp
sed -i 's/\r$//g'    /opt/tomcatidc/ipscpdiskcleanpp
...
iplist=`cat /opt/tomcatidc/ipscpdiskcleanpp`
...
sleep 5s
cp /opt/tomcatidc/ipscpdiskcleanpp /opt/tomcatidc/ipscpdiskcleanmulu/
cd  /opt/tomcatidc/ipscpdiskcleanmulu
hangshu=`cat /opt/tomcatidc/ipscpdiskcleanmulu/ipscpdiskcleanpp  |wc -l`

if [ $hangshu -gt 0 ] && [ $hangshu -le 100 ];then
split -l 3  /opt/tomcatidc/ipscpdiskcleanmulu/ipscpdiskcleanpp -a 4 xa
elif [ $hangshu -gt 100 ] && [ $hangshu -le 800 ];then
split -l 8 /opt/tomcatidc/ipscpdiskcleanmulu/ipscpdiskcleanpp -a 4 xa

elif [ $hangshu -gt 800 ] && [ $hangshu -le 1500 ];then

split -l 15  /opt/tomcatidc/ipscpdiskcleanmulu/ipscpdiskcleanpp -a 4 xa
elif [ $hangshu -gt 1500 ] && [ $hangshu -le 2000 ];then
split -l 20  /opt/tomcatidc/ipscpdiskcleanmulu/ipscpdiskcleanpp -a 4 xa

else
split -l 70  /opt/tomcatidc/ipscpdiskcleanmulu/ipscpdiskcleanpp -a 4 xa
fi


#split -l 70 /opt/tomcatidc/webapps/jcIDCpiliangcaozuo/upload/qiegeipjianyantest/ipjavajianyan.txt -a 4 xa
sleep 12s
ln=`ls -l /opt/tomcatidc/ipscpdiskcleanmulu/xa*|wc -l`
ipqiegelist=`ls -l /opt/tomcatidc/ipscpdiskcleanmulu/xa*|awk '{print $9}'`
sleep 8s
...
do
#ipqielistttt=`cat $num`
nohup /opt/tomcatidc/scpdiskjihuarenwu10s.sh $num  >> /opt/tomcatidc/wasdatasourceipjavalogzong.log &
done
sleep 115s
