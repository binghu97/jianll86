...
if [[ -n $zstartupsh1  ]] && [[ -n $zstartsh1 ]]&&[[ -n $wenjiantomcat1 ]]
then
#start tomcat with start.sh
	cd  $A1zupstartup && grep -rn  jdbc: ../ > /home/wasadmin/datasourcetomcat.log
elif [[ -n $zstartupsh2  ]] && [[ -n $zstartsh2 ]]&&[[ -n $wenjiantomcat2 ]]
then
#start tomcat with start.sh
        cd  $A2zupstartup && grep -rn  jdbc: ../ > /home/wasadmin/datasourcetomcat.log
elif [[ -n "$zstartupsh1"  ]] && [[ ! -n "$zstartsh1" ]]&&[[ -n "$wenjiantomcat1" ]]
then
...
#start tomcat without start.sh
        cd  $A2zupstartup && grep -rn  jdbc: ../  > /home/wasadmin/datasourcetomcat.log
elif [[  -n "$zstartshjar1"  ]] && [ -n "$wenjian3" ] && [ -n "$jarpanduan1" -o  -n "$jarpanduan2" ]
then
#start jar without parameter
         cd  $czupstartsh && cp *.jar  /home/wasadmin/
         cd  $czupstartsh && cp ../jar/*.jar  /home/wasadmin/
elif [  -n "$zstartshjar2"  ] && [ -n "$wenjian4" ] && [ -n "$jarpanduan3" -o  -n "$jarpanduan4" ]
then
#start jar without parameter
         cd  $Dzupstartsh && cp *.jar  /home/wasadmin/
         cd  $Dzupstartsh && cp ../jar/*.jar  /home/wasadmin/
elif [[ -n $zstartshjar1 ]]&&[[ -n $wenjian3 ]]&&[[ ! -n $jarpanduan1 ]]&&[[ ! -n $jarpanduan2 ]]
then
#start jar with parameter
	cd  $czupstartsh && cp *.jar  /home/wasadmin/
         cd   $czupstartsh && cp ../jar/*.jar  /home/wasadmin/
elif [[ -n $zstartshjar2 ]]&&[[ -n $wenjian4 ]]&&[[ ! -n $jarpanduan3 ]]&&[[ ! -n $jarpanduan4 ]]
then
#start jar with parameter
cd  $Dzupstartsh && cp *.jar  /home/wasadmin/
cd  $Dzupstartsh && cp ../jar/*.jar  /home/wasadmin/
